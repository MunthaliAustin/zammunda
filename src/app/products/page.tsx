...
export default function ProductsPage() {
  ...
  }, [user, isLoading, selectedCategory, priceRange, sortBy, searchQuery]);
    ...
    try {
      ...
      const productsWithInventory = await Promise.all(
        productData.map(async (product) => {
          try {
            const inventory = await inventoryService.getInventoryBySkuCode(product.skuCode);
                  ...
                  <div className="flex-1 p-5 md:p-6">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="text-right">
                        <span className="text-2xl font-bold text-green-600 block mb-3">MWK {product.price.toFixed(2)}</span>
                        <button 
                          onClick={() => router.push(`/products/${product.id}`)}
